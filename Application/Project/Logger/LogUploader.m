...
- (void)uploadLogFile:(NSArray *)sortedFiles
    ...
    for (NSString *logFile in sortedFiles) {
        ...
        if (components && [components count] > 0) {
            ...
            /*
             // MOBILE_DEBUG defined in 'Building Settings-->Preprocessor Macros'
             if (MOBILE_DEBUG) {
             NSString *debugInfoStr = [components objectAtIndex:0];
             NSArray *debugInfoList = [debugInfoStr componentsSeparatedByString:LOG_DEBUG_SEPARATOR];
             if (debugInfoList.count == 2) {
             logTime = [debugInfoList objectAtIndex:1];
             } else {
             continue;
             }
             } else {
             logTime = [components objectAtIndex:0];
             }
             */
