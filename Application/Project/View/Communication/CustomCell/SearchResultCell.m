...
CellMargin SRCM = {10.f, 10.f, 10.f, 10.f};
...
-(void)prepareForReuse
{
    ...
    self.verLabel.text = @"";;
    self.portImageView.image = IMAGE_WITH_IMAGE_NAME(@"groupCellDefaultHeader.png");
}
...
//    self.layer.shadowPath = [UIBezierPath bezierPathWithRect:self.bounds].CGPath;
    ...
    _portImageView = [[GHImageView alloc] initWithFrame:CGRectMake(SRCM.left, SRCM.top, IMAGEVIEW_WIDTH, IMAGEVIEW_HEIGHT) defaultImage:@"groupCellDefaultHeader.png"];
...
//    backView.layer.shadowPath = [UIBezierPath bezierPathWithRect:backView.bounds].CGPath;
    ...
    CGFloat titleLabelOriginX = SRCM.left * 2 + IMAGEVIEW_WIDTH;
    CGFloat titleLabelWidth = [self cellWidth] - titleLabelOriginX - SRCM.right;
    
    _titLabel = [[UILabel alloc] initWithFrame:CGRectMake(titleLabelOriginX, SRCM.top, titleLabelWidth, 20)];
    ...
    _desLabel = [[UILabel alloc] initWithFrame:CGRectMake(titleLabelOriginX, SRCM.top + _titLabel.frame.size.height, titleLabelWidth, 20)];
...
//    CGFloat versionLabelOriginX = [self cellWidth] - VERLABEL_WIDTH - SRCM.right;
//    CGFloat versionLabelOriginY = [self cellHeight] - VERLABEL_HEIGHT - SRCM.bottom - 5;
//    
...
- (void)updateCell:(UserProfile *)userProfile{
    ...
    UserBaseInfo *ubf = [CommonMethod userBaseInfoWithUserProfile:userProfile];
    [ubf parseValueProperties];
    
    [self.portImageView updateImage:ubf.portraitName];
    self.titLabel.text = [NSString stringWithFormat:@"%@|%@",ubf.chName, ubf.enName];
    self.desLabel.text = [NSString stringWithFormat:@"%@ %@ %@",ubf.company,ubf.department,ubf.position];
