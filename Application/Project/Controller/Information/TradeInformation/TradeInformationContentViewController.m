...
@interface TradeInformationContentViewController ()<UIWebViewDelegate, FTPHelperDelegate,SSZipArchiveDelegate,WXWCustomeAlertViewDelegate, UIActionSheetDelegate, WXApiDelegate, CircleMarketingEventCommentViewControllerDelegate> {
...
-(void)loadSpecifieldIDInfo:(LoadTriggerType)triggerType forNew:(BOOL)forNew
{
    ...
    [specialDict setObject:NUMBER(_specialId) forKey:KEY_API_PARAM_SPECIFIED_ID];
...
- (void)addBottomView {
    ...
    _readLabel = [[UILabel alloc] initWithFrame:CGRectMake(readImageView.frame.size.width + readImageView.frame.origin.x, readOriginY, 90, BUTTON_READ_COMMENT_HEIGHT)];
...
- (void)updateCommentCount:(int)count
{
    ...
    [[ProjectDBManager instance] updateInformationCommentCount:_infoID count:count];
}
...
- (void)updateLabelTexts:(InformationList *)infoList {
    ...
    self.readLabel.text = [NSString stringWithFormat:@"阅读:%d", [[ProjectDBManager instance] getInformationCommentReader:_infoID]];
    
    ;
    self.commentLabel.text = [NSString stringWithFormat:@"评论:%d", [[ProjectDBManager instance] getInformationCommentCount:_infoID]];
}
    ...
    if (buttonIndex == [actionSheet cancelButtonIndex]) {
        ...
        if ([WXApi isWXAppInstalled] && [WXApi isWXAppSupportApi]) {
            BOOL reult = [CommonUtils shareByWeChat:buttonIndex
                                        ...
                                        description:@""
                                                url:[NSString stringWithFormat:@"%@%d",[[ProjectDBManager instance] getCommon:INFORMATION_SHARE_WEIXIN_KEY], _infoID]];
            if (reult) {
    ...
    switch (contentType) {
        ...
        case UPDATE_READER_TY:
        {
            ...
            if (ret == SUCCESS_CODE) {
                ...
                [[ProjectDBManager instance] updateInformationCommentReader:_infoID count:reader];
