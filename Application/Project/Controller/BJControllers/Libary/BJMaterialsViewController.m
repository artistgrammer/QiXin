...
@interface BJMaterialsViewController ()
{
    
}
@end

@implementation BJMaterialsViewController
...
//        self.view = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil].view;
        ...
        mMaterialsArray = [[NSMutableArray alloc] initWithCapacity:10];
    }
    return self;
}
...
- (void)adjustTableLayout {
    _tableView.frame = CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT - NAV_BAR_HEIGHT - TAB_BAR_HEIGHT);
}
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"Metarial";
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [mMaterialsArray count]/*CELL_COUNT**/;
}


- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *CustomCellIdentifier =@"BJMetarialsViewCellIdentifier";
...
- (void)loadAllLibrarys
{
    ...
    WXWAsyncConnectorFacade *connFacade = [self setupAsyncConnectorForUrl:PG_SERVER_URL_GETMETERIALS
                                                              contentType:PG_GET_ALLMETERIALS];
    [connFacade post:PG_SERVER_URL_GETMETERIALS data:[specialDict JSONData]];
}
        ...
        case PG_GET_ALLMETERIALS:
        {
            ...
            if (ret == SUCCESS_CODE) {
                ...
                if ([listArr count] == 0) {
                    ...
                    if ([mMaterialsArray count] > 0) {
                        [mMaterialsArray removeAllObjects];
                    }
                }
//
