...
- (void)_initCommonUI
{
    ...
    self.backgroundColor = (_tintColor = [[UIColor clearColor] retain]);
    //self.backgroundColor = [UIColor blackColor];
    _pages = (_items.count / kPageMenuItemCount) + (_items.count % kPageMenuItemCount == 0 ? 0 : 1);
...
- (void)_resetContainerViewLayout
{
    if(_items.count <= kPageMenuItemCount/2){
...
- (void)_resetSubViewsLayout
{
    ...
    for(int page = 0; page < _pages; page ++){
        ...
        for(int index = (page * kPageMenuItemCount); index < (page * kPageMenuItemCount + kPageMenuItemCount); index++){

            if(index == _items.count) break;            
            UIMenuBarItem *item = (UIMenuBarItem *)[_items objectAtIndex:index];
            
            int relativeIndex = index - (page * kPageMenuItemCount);
            int row = relativeIndex/(kPageMenuItemCount/2) < 1? 0 : 1;
            int coloumn = (relativeIndex % (kPageMenuItemCount/2));
            
            int width = self.frame.size.width/(kPageMenuItemCount/2);
