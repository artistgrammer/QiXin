...
- (void)viewDidLoad
{
    ...
    for (int i = 0; i< 5; i++)
    {
        ...
        img = [[UIImageView alloc] initWithFrame:CGRectMake(18.0f, 37.0f+(7+28)*i, 244.0f, 28.0f)];
        img.image = [UIImage imageNamed:@"input_box@2x.png"];
        [self.view addSubview:img];
        [img release];
        
        UITextField *mtextField = [[UITextField alloc] initWithFrame:CGRectMake(28.0f, 37.0f+(7+28)*i, 234.0f, 28.0f)];
    ...
    chooseContactsBtn.frame = CGRectMake(275.f, 37, 28, 28);
    [chooseContactsBtn addTarget:self action:@selector(goChooseMultiPhoneNumbers) forControlEvents:UIControlEventTouchUpInside];
    ...
    for (UIView * view in self.view.subviews)
    {
        if (view.tag >= 800)
        {
            if ([view isKindOfClass:[UITextField class]])
            {
                ...
                int myIndex = view.tag - 800;
                if (myIndex >= 0 && myIndex < [phoneNumbers count])
                {
                    ContactPhone *contact = [phoneNumbers objectAtIndex:myIndex];
