       ...
       detailType:(int)type
{
    if (self = [super initWithMOC:MOC]) {
        ...
        _eventId = [eventList.eventId integerValue];
        _type = type;
    }
    return self;
}
...
- (void)addBottomView {
    ...
    bottom.backgroundColor = [UIColor colorWithPatternImage:IMAGE_WITH_NAME(@"navBar_bg1.png")];
    [self.view addSubview:bottom];
    
    CGFloat readOriginY = (BOTTOM_HEIGHT - BUTTON_READ_COMMENT_HEIGHT) / 2.f;
    
    UIImageView *readImageView = [[UIImageView alloc] initWithFrame:CGRectMake(MARGIN_LEFT, readOriginY, BUTTON_READ_COMMENT_WIDTH, BUTTON_READ_COMMENT_HEIGHT)];
    readImageView.image = IMAGE_WITH_NAME(@"information_read");
    [bottom addSubview:readImageView];
    
    UILabel *readLabel = [[UILabel alloc] initWithFrame:CGRectMake(readImageView.frame.size.width + readImageView.frame.origin.x, readOriginY, 90, BUTTON_READ_COMMENT_HEIGHT)];
    ...
    commentImageView.image = IMAGE_WITH_NAME(@"information_comment");
    [bottom addSubview:commentImageView];
    
    _commentLabel = [[UILabel alloc] initWithFrame:CGRectMake(commentImageView.frame.size.width + commentImageView.frame.origin.x, commentOriginY, 90, BUTTON_READ_COMMENT_HEIGHT)];
    ...
    UIImageView *shareImageView = [[UIImageView alloc] initWithFrame:CGRectMake(shareLabel.frame.origin.x - BUTTON_SHARE_WIDTH, shareOriginY, BUTTON_SHARE_WIDTH, BUTTON_SHARE_HEIGHT)];
...
- (void)comment:(UIButton *)sender {
    CircleMarketingEventCommentViewController *vc = [[[CircleMarketingEventCommentViewController alloc] initWithMOC:_MOC parentVC:self commentType:CommentType_Event paramId:_eventId] autorelease];
    vc.delegate = self;
    [CommonMethod pushViewController:vc withAnimated:YES];
...
- (void)loadListData:(LoadTriggerType)triggerType forNew:(BOOL)forNew {
    ...
    if (_type == 0) _currentType = LOAD_EVENT_DETAIL_PRE_TY;
    else _currentType = LOAD_EVENT_DETAIL_REV_TY;
    ...
    [specialDict setObject:NUMBER(_eventId) forKey:KEY_API_PARAM_EVENT_ID];
...
- (void)loadSubmitApplyData:(LoadTriggerType)triggerType forNew:(BOOL)forNew withApplyList:(NSArray *)applyList {
    ...
    [specialDict setObject:[NSString stringWithFormat:@"%d",_eventId] forKey:KEY_API_PARAM_EVENT_ID];
...
- (void)configureMOCFetchConditions {
    ...
    //    DLog(@"detail: %d",_eventId);
    self.predicate = [NSPredicate predicateWithFormat:@"eventId == %d",_eventId];
...
//                NSString *timestamp = OBJ_FROM_DIC(resultDic, @"timestamp");
                ...
                //                [[ProjectDBManager instance] upinsertCircleMarketing:eventListArray timestamp:[timestamp doubleValue]];
                
                if (eventListArray && eventListArray.count) {
                    for (EventDetailList * event  in eventListArray) {
                        if ([event.eventId integerValue] == _eventId) {
...
#pragma mark -- imagewallscrollview
        ...
        _imageWallScrollView.autoPlay = NO;
//        _imageWallScrollView.autoScrollDelayTime = 2.f;
...
- (UITableViewCell *)drawMoreCell:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath {
    ...
    [moreButton setTitleColor:[UIColor colorWithHexString:@"0xe83e0b"] forState:UIControlStateNormal];
...
- (void)circleMarketingDetailTitleCell:(CircleMarketingDetailTitleCell *)cell leftButtonClicked:(UIButton *)leftButton {
    CircleMarketingMemberListViewController *vc = [[[CircleMarketingMemberListViewController alloc] initWithMOC:_MOC parentVC:self eventId:_eventId] autorelease];
    [CommonMethod pushViewController:vc withAnimated:YES];
}
...
- (void)popupSuccessAlertView
{
    ...
    _eventList.applyStatus = NUMBER(1);
