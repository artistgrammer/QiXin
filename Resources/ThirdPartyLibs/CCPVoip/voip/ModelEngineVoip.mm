...
- (void)onConnectError:(NSInteger)reason withReasonMessge:(NSString *)reasonMessage
{
    if (reason == EReasonKickedOff) {
        ...
        {
            // TODO code from VoipCode.plist
            /*
            NSString *resourcePath = [[NSBundle mainBundle] resourcePath];
            NSString *filePath = [resourcePath stringByAppendingPathComponent:@"VoipCode.plist"];
            NSDictionary *VoipCodeDict = [NSDictionary dictionaryWithContentsOfFile:filePath];
            NSString *errDesc = [VoipCodeDict objectForKey:@(reason)];
            */
...
-(BOOL)onUdpSocket:(AsyncUdpSocket *)sock didReceiveData:(NSData *)data withTag:(long)tag fromHost:(NSString *)host port:(UInt16)port{
    ...
    [inputFormatter setDateFormat:kDATE_TIME_FORMAT];
