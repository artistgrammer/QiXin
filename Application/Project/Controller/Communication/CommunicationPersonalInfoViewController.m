...
- (id) initWithUserId:(NSString*)userId withDelegate:(id)delegat
{
    if (self = [super init]) {
        _userId = userId;
...
- (void)initData {
    ...
    userObject = [[ProjectDBManager instance] getUserInfoFromDB:_userId];
    _isFriend = userObject.isFriend;
}
...
- (void)addMainTable {
    ...
    if (self.numberOfRows.count > 0) {
        _mainTable.tableFooterView = [self tableViewForFooterWithBool:userObject.isFriend];
    }
...
- (UIView *)tableViewForFooterWithBool:(BOOL)bol {
    ...
    if ([_userId isEqualToString:[AppManager instance].userId]) {
            ...
            if (ret == SUCCESS_CODE) {
                ...
                if (userObject.isFriend) {
                    if (_delegate && [_delegate respondsToSelector:@selector(deleteFriendUser:)]) {
                        [_delegate deleteFriendUser:userObject.userId];
                    }
                }
                ...
                [[ProjectDBManager instance] updateIsFriendWithId:_userId isFriend:_isFriend];
...
- (void)loadSubmitPrivateLetter:(LoadTriggerType)triggerType forNew:(BOOL)forNew type:(enum FRIEND_TYPE)type
{
    ...
    [specialDict setObject:_userId forKey:KEY_API_PARAM_USERID];
...
- (void)sendMessage:(UIButton *)sender {
    ...
    if (_isFromChatVC) {
        ...
        ChatDetailViewController *vc = [[[ChatDetailViewController alloc] initWithPrivateDataWithUserBaseInfo:[[ProjectDBManager instance] getUserInfoFromDB:_userId] withType:CHAT_TYPE_PRIVATE MOC:_MOC] autorelease];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        CGFloat height = [self cellHeightWithTitle:userObject.userName titleFont:FONT_TITLE subTitle:userObject.userName subTitleFont:FONT_SUBTITLE] + 10;
        return height;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell) {
        ...
        if (indexPath.section == 0 && indexPath.row == 0) {
            ...
            //            [cell.headerImage updateImage:dbprop0.propValue];
