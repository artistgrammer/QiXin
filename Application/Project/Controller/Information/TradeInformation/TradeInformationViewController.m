...
- (void)viewDidLoad
{
    ...
    [self initSearchBar];
    SwitchTabBar *tabBar = [[SwitchTabBar alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 40.f)
                                                    titleArray:@[@"行业资讯", @"推荐资讯"]
                                              hasSeparatorLine:NO
                                                      delegate:self];
    
    tabBar.backgroundColor = [UIColor colorWithHexString:@"0xd5d5d5"];
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, tabBar.frame.size.height + self.searchBar.frame.size.height)];
    CGRect f = self.searchBar.frame;
    f.origin.y = tabBar.frame.size.height;
    self.searchBar.frame = f;
                ...
                if (contentDic) {
                    ...
                    if ( [isAll isEqualToString:@"0"]) {
                        ...
                        if (![contentList isEqual:[NSNull null]] && contentList.count) {
                            ...
                            [[ProjectDBManager instance] deleteOldInfomationFromTimestamp:lastUpdateTime];
...
- (void)configureMOCFetchConditions {
    ...
    if (isSearched) {
        ...
        self.predicate = [NSPredicate predicateWithFormat:@"isDelete == 0 and informationID != 0 and sortOrder > 0"];
    } else{
...
- (void)loadListData:(LoadTriggerType)triggerType forNew:(BOOL)forNew withKeyword:(NSString *)keyword {
    ...
    if (forNew)
    {
        ...
        [specialDict setObject:[CommonMethod convertLongTimeToString:[[ProjectDBManager instance] getOldestInfomationTimestamp] / 1000 ]  forKey:KEY_API_PARAM_END_TIME];
    }
