...
- (void)initValues
{
    ...
    _topScrollView.backgroundColor = [UIColor  colorWithHexString:@"0xe6e6e6"];
...
- (void)createNameButtons
{
    ...
    //每个tab偏移量
    CGFloat xOffset = 17.0f;
    for (int i = 0; i < [self.m_tabArr count]; i++)
    {
//        UIViewController *vc = m_tabArr[i];
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        CGSize textSize = [[m_tabArr objectAtIndex:i] sizeWithFont:[UIFont systemFontOfSize:kFontSizeOfTabButton]
        ...
        [button setTag:i+100];
        if (i == 0) {
            _shadowImageView.frame = CGRectMake(17.0, 0, textSize.width, kHeightOfTopScrollView);
            button.selected = YES;
        }
        [button setTitle:[m_tabArr objectAtIndex:i] forState:UIControlStateNormal];
