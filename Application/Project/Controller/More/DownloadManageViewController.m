...
- (void)loadData {
    ...
    NSArray *downloadingArr = [[ProjectDBManager instance] getAllDownloadingCourseDetail:_MOC];
    ...
    for (int i = 0; i < downloadingArr.count; i++) {
        ...
        if (nil != cdl.chapterLists && cdl.chapterLists.count) {
            for (ChapterList *cl in [cdl.chapterLists.allObjects sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"chapterID" ascending:YES]]]) {
                if (![[ProjectDBManager instance] isDownloading:cl.courseID.integerValue withChapterId:cl.chapterID.integerValue]) {
    ...
    self.downloadedInfos = [[ProjectDBManager instance] getAllDownloadedCourseDetail:_MOC];
    
//    NSArray *downloadedArr = [[ProjectDBManager instance] getAllDownloadedCourseDetail:_MOC];
//    
//    for (int i = 0; i < downloadedArr.count; i++) {
//        CourseDetailList *cdl = (CourseDetailList *)downloadedArr[i];
//        if (nil != cdl.chapterLists && cdl.chapterLists.count) {
//            for (ChapterList *cl in [cdl.chapterLists.allObjects sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"chapterID" ascending:YES]]]) {
//                if (![[ProjectDBManager instance] isDownloaded:cl.courseID.integerValue withChapterId:cl.chapterID.integerValue]) {
//                    [cdl removeChapterListsObject:cl];
//                }
//            }
...
- (void)addDeleteView {
    _deleteView = [[UIView alloc] initWithFrame:CGRectMake(0, SCREEN_HEIGHT, self.view.frame.size.width, 50.f)];
...
- (void)hideDeleteView {
    if ((self.deleteView.frame.origin.y + self.deleteView.frame.size.height + TABBAR_HEIGHT + 10) == SCREEN_HEIGHT) {
        [UIView animateWithDuration:.3f delay:0
                            options:UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseInOut
                         animations:^{
            self.deleteView.frame = CGRectMake(self.deleteView.frame.origin.x,
    ...
    for (ChapterList *cl in chapters) {
        if ([cl.chapterFileURL hasSuffix:@"zip"]) {
            if ([CommonMethod isExist:[CommonMethod getChapterZipFilePath:cl]]) {
                [[NSFileManager defaultManager] removeItemAtPath:[CommonMethod getChapterZipFilePath:cl] error:nil];
                [[NSFileManager defaultManager] removeItemAtPath:[CommonMethod getChapterZipFileFolderPath:cl] error:nil];
                [[ProjectDBManager instance] deleteChapterWithChapterId:cl.chapterID.integerValue courseId:cl.courseID.integerValue];
            }
        }else {
...
- (DownloadingCell *)drawDownloadingCell:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *downloadCellID = [NSString stringWithFormat:@"downloading_cell_%d",indexPath.row];
...
- (DownloadedCell *)drawDownloadedCell:(UITableView *)tableView atIndexPath:(NSIndexPath *)indexPath {
    ...
    ChapterList *list = (ChapterList *)[[cl.chapterLists.allObjects sortedArrayUsingDescriptors:@[[NSSortDescriptor sortDescriptorWithKey:@"chapterID" ascending:YES]]] objectAtIndex:indexPath.row];
    DLog(@"downloaded chapter title:%@",list.chapterTitle);
    
    BOOL isSelected = NO;
    
    if (self.chapters.count) {
        for (ChapterList *cl in self.chapters) {
            if (cl.chapterID.integerValue == list.chapterID.integerValue) {
                isSelected = YES;
            }
        }
    }
    
    [cell drawDownloadedCell:list selected:isSelected];
