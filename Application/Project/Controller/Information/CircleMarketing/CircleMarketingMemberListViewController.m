...
- (UIView *)initMemberList:(NSArray *)array withCount:(int)count
{
    int marginX = 0;
    int marginY = 5;
    int distX = (SCREEN_WIDTH - 2*_marginX - 4*COMMUNICATION_MEMBER_HEADER_BRIEF_VIEW_WIDTH ) / (COMMUNICATION_MEMBER_HEADER_BRIEF_VIEW_MAX_COLUMN_COUNT - 1);
    int distY = 5;
    ...
    //-------------------------------------------------------
    view.frame = CGRectMake(0, 0, SCREEN_WIDTH - 2*self.marginX, startY + height + marginY);
...
- (UIScrollView *)initScrollView
{
    ...
    int height = SCREEN_HEIGHT - ITEM_BASE_TOP_VIEW_HEIGHT -   - SYSTEM_STATUS_BAR_HEIGHT - 44;
