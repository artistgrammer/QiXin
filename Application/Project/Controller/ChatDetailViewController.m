...
@interface ChatDetailViewController () <UITextFieldDelegate, UIScrollViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate,HPGrowingTextViewDelegate, ECClickableElementDelegate, ChatMenuViewDelegate,MapViewDelegate> {
    ...
    IMGroupInfo *_chatGroupDataModal;
...
@property (nonatomic, retain) IMGroupInfo *groupModal;
...
- (id)initWithData:(IMGroupInfo *)groupModal withType:(enum CHAT_TYPE)type MOC:(NSManagedObjectContext *)MOC
{
    if ((self = [super initWithMOC:MOC needRefreshHeaderView:NO needRefreshFooterView:NO])) {
        
        self.groupModal = groupModal;
        
        self.receiver = self.groupModal.groupId;
       
        chatType = type;
        self.navigationItem.title = self.groupModal.name;
    }
...
- (void)adjustTableView
{
    ...
    [CommonMethod viewAddGuestureRecognizer:self.tableView withTarget:self withSEL:@selector(chatListTapped:)];
}
...
- (void)notifierNetworkStatus:(NSNotification *)notification {
    ...
    if (notification.userInfo != nil) {
        ...
        NSString *naviStr = self.groupModal.name;
        switch (status) {
...
{
    UIView *view = [[[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - BOTTOM_BAR_HEIGHT , IPHONE_WIDTH, BOTTOM_BAR_HEIGHT)] autorelease];
...
{
    _bottomChatView = [[UIImageView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 64 - BOTTOM_BAR_HEIGHT, IPHONE_WIDTH, BOTTOM_BAR_HEIGHT)];
...
- (void)setExpressionBtn:(UIView *)parentView
{
    ...
    for (int i = 1; i< 3; i++ )
    {
        ...
        [btn setFrame:CGRectMake(IPHONE_WIDTH - wGap - width*i - xGap*(i - 1), (_bottomChatView.frame.size.height - height)/2, width, height)];
...
{
    _chatMenuView = [[ChatMenuView alloc]initWithFrame:CGRectMake(0, IPHONE_HEIGHT, IPHONE_WIDTH, KEYBOARD_HEIGHT)];
...
- (void)initMicView
{
    ...
    int height = 72;
    int width = 72;
    int startX = (IPHONE_WIDTH - width ) / 2.0f;
    int startY = (IPHONE_HEIGHT - width) / 2.0f - 50;
...
- (void)autoMovekeyBoard:(CGFloat)height withDuration:(NSTimeInterval)duration {
    ...
    [UIView animateWithDuration:duration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
      ...
      _bottomChatView.frame = CGRectMake(0.0f, (self.view.frame.size.height - height - bottomTabHeight), IPHONE_WIDTH, bottomTabHeight);
    
        
        //Other Funtion View
        if (_isOtherFuntion) {
            [_chatMenuView setFrame:CGRectMake(0, IPHONE_HEIGHT - KEYBOARD_HEIGHT - 64, IPHONE_WIDTH, KEYBOARD_HEIGHT)];
            [self.view bringSubviewToFront:_chatMenuView];
        } else {
            [_chatMenuView setFrame:CGRectMake(0, IPHONE_HEIGHT, IPHONE_WIDTH, KEYBOARD_HEIGHT)];
        }
        ...
        // Chat TableView
        if (height == 0.f) {
            self.tableView.frame = CGRectMake(0.f, height, self.tableView.frame.size.width, (IPHONE_HEIGHT - ITEM_BASE_BOTTOM_VIEW_HEIGHT - SYS_STATUS_BAR_HEIGHT - bottomTabHeight));
        } else {
...
- (void)sendTxtMsg {
    ...
    NSString *cid = [self.modelEngineVoip sendInstanceMessage:self.receiver andText:msgTxt andAttached:nil andUserdata:nil];
    if (cid.length > 0)
    {
        msg.msgid = cid;
...
// Send Voice
- (void)sendVoiceMsg {
    self.curRecordVoiceMsgId = [self.modelEngineVoip sendInstanceMessage:self.receiver andText:nil andAttached:self.sendPath andUserdata:@"cloopen"];
}
