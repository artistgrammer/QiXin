...
- (CGFloat)getCellHeightWithMsg:(IMMessageObj*)msg
{
    ...
    {
        CGSize size = [msg.content sizeWithFont:[UIFont systemFontOfSize:R_CONTENT_FONT_SIZE] constrainedToSize:CGSizeMake(R_CONTENT_WIDTH, 3000.0f) lineBreakMode:UILineBreakModeCharacterWrap];
        heigt = size.height + 50.0f;
    }
...
- (void) onSendInstanceMessageWithReason: (CloopenReason *) reason andMsg:(InstanceMsg*) data
{
    ...
    {
        [self  popPromptViewWithMsg:@"发送文本过长！" AndFrame:CGRectMake(0, 160, 320, 30)];
        return;
    }
//    else if (reason != 0 )
//    {        
...
{
    isPlaying = NO;
    [self.bubbleVoiceImage stopAnimating];
...
-(void)playVoiceMsg:(id)sender
{
    ...
    self.bubbleVoiceImage = (UIImageView*)[contentview viewWithTag:1005];
...
{
    isPlaying = YES;
    [self.bubbleVoiceImage startAnimating];
